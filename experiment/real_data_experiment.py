...
def check_convergence(score_list: list, score_type: str='NLL') -> bool:
    ...
    if score_type == 'NLL':
        if len(score_list) < 5:
            return False
        if score_list[-1] < -2500:
            good_iterations = sum(1 for i in score_list if i < -2500)
...
def randomize_data(
    ...
    random_indices = [int(i) for i in permutation_id if i not in fixed_indices]
    random_indices = np.random.permutation(random_indices)
    for i in range(len(permutation_id)):
        if i == 0 and known_initial_value:
            continue  # Skip the first index if initial value is known
...
def run_experiment(
    ...
    if random_percent > 0:
        random_X, permutation_id = randomize_data(real_data, known_initial_value=known_initial_value,
                                random_percent=random_percent, random_seed=17)
    else:
        random_X = np.copy(real_data)
        permutation_id = np.arange(random_X.shape[1])

    right_percent = check_sorting_accuracy(real_data, random_X, check_by_indices_order=True,
                                            reordered_order=permutation_id)
    print(right_percent, "Right percent before estimation")
    ...
    estimated_A, estimated_H, reordered_X, best_ordered_data, all_nlls, all_nlps, all_mae_a, all_mae_h, all_right_percent = \
        estimate_sde_parameters(random_X, real_data, reordered_order=permutation_id, time_step_size=dt, T=T, max_iter=max_iter, true_A=A,
    ...
    for i in range(real_data.shape[0]):
        ...
        if i <= 5:
            ax1.plot(time, real_data[i, :, 0], label=f"Trajectory {i}")
    ...
    plt.savefig('figures_real_data/progress.png')
    ...
    plt.savefig('figures_real_data/reordering_accuracy.png')

    return


# function for loading data from molecule trajectory data
def load_molecule_data(folder:str = "real_data/Evolution_Trajectory/hanshi_data/feature1"
                       ) -> tuple[np.ndarray, list[str]]:
    """
    Read all *.gdf files in *folder* and return a float array with shape
    (n_particles, n_frames, 3).  Frames that do not match the majority
    particle count are skipped and reported.

    Parameters
    ----------
    folder : str or Path
        Directory that contains `feature*.gdf` files.

    Returns
    -------
    traj : ndarray  (n_particles, n_frames, 3)
    bad  : list[str]
        Filenames that were ignored because of length mismatch.
    """
    folder = Path(folder)
    gdf_files = sorted(folder.glob("feature*.gdf"))
    if not gdf_files:
        raise FileNotFoundError("no *.gdf files found in {}".format(folder))

    frames   = []
    bad      = []
    expected = None          # number of particles we *want* to see

    for f in gdf_files:
        # --- read 6-word header (24 bytes) -------------------------------
        hdr = np.fromfile(f, dtype="<u4", count=6)
        # if hdr.size < 6:
        #     bad.append(f.name);  continue
        payload_len = int(hdr[5])                 # how many float32 numbers follow
        # if payload_len % 3 != 0:
        #     bad.append(payload_len);  continue         # not xyz triples

        n_particles = 2000
        flat = np.fromfile(f, dtype="<f4", count=n_particles*3, offset=24)

        # --- set or check the expected particle count -------------------
        if expected is None:
            expected = n_particles
        # if n_particles != expected:
        #     bad.append(f.name);  continue
        if np.any(flat > 50000):
            continue

        frames.append(flat.reshape(expected, 3))

    if not frames:
        raise RuntimeError("Every file in {} failed the consistency check!".format(folder))

    # stack → (n_frames, n_particles, 3)  ➔ swap → (n_particles, n_frames, 3)
    traj = np.swapaxes(np.stack(frames, axis=0), 0, 1)
    return traj, bad


def load_fred_data(file_path: str = "real_data/DGS10.csv") -> np.ndarray:
    """Load FRED data from a CSV file.
    ...
    """
    # FRED DSG10 Data Loading
    # --- 1. Load the CSV ---------------------------------------------------------
    df = pd.read_csv(file_path,
                    usecols=[0, 1], # use only the first two columns (date and value)
                    na_values=[".", "NaN", ""]) # treat these as missing

    # 2. Parse the first column (dates); coerce invalid strings to NaT
    df.iloc[:, 0] = pd.to_datetime(df.iloc[:, 0], errors="coerce")

    from functools import partial
    f = partial(pd.to_datetime, dayfirst=False)
    mask = (df.iloc[:, 0] >= f("1963-01-01")) & (df.iloc[:, 0] <= f("1964-04-30")) # 751
    subset = df.loc[mask, df.columns[1]].dropna().to_numpy(dtype=float)
    subset = subset[:22*15] # 750 instead of 751
    print(len(subset), "length of subset")

    # # --- 3. Trim so the length is a multiple of 730 -----------------------------
    # num_complete_samples = subset.size // 730
    # clean_values = subset[: num_complete_samples * 730]

    # --- 4. Reshape to (num_sample, 730, 1) -------------------------------------
    data = subset.reshape(int(len(subset)/15), 15, 1)

    print(data.shape)        # e.g. (173, 730, 1)
    retain = []
    for i in range(data.shape[0]):
        if data[i, 0, 0] - data[i, -1, 0] < 0:
            retain.append(i)
    data = data[retain]
    print(data.shape, "after removing the decreasing ones") # e.g. (173, 730, 1)
    # data = np.concatenate((data, data), axis=2)

    # temp = np.copy(data)
    # for i in range(50):
    #     temp = np.concatenate((temp, data + i/50), axis=0)
    # data = temp
    # print(data.shape, "after concatenating")

    return data


def load_exchange_rate_data(
        file_path: str = "real_data/all_six_datasets/exchange_rate/exchange_rate.csv"
        ) -> np.ndarray:
    """Load exchange rate data from a CSV file.
    Args:
        file_path (str): Path to the CSV file.
    Returns:
        np.ndarray: Loaded data as a NumPy array.
    ...
    # Extract the 8 columns of interest (excluding 'date' and 'OT' column)
    data = df.iloc[:, 1:].to_numpy()  # shape: (total_days, 8)

    # Set parameters
    num_steps = 15
    num_features = 8
    num_trajectories = int(np.ceil(data.shape[0]/num_steps))

    # Pad the data if necessary to make it divisible by num_steps
    padding_length = num_steps * num_trajectories - data.shape[0]
    if padding_length > 0:
        padding = np.full((padding_length, num_features), np.nan)
        data = np.vstack([data, padding])

    # Reshape the data into (-1, num_steps, 8)
    data = data.reshape(-1, num_steps, num_features)

    print("Reshaped data shape:", data.shape)

    # If not wanted, remove the NaNs data
    data = data[~np.isnan(data).any(axis=(1, 2))]  # Remove trajectories with NaNs
    ...
    for i in range(data.shape[0]):
        if data[i, 0, 0] - data[i, -1, 0] < 0:
            retain.append(i)
    data = data[retain]
    print(data.shape, "after removing the decreasing ones")

    return data


def load_electricity_data(
        file_path: str = "real_data/electricity/electricity.csv"
    ) -> np.ndarray:
    """Load electricity data from a CSV file.
    Args:
        file_path (str): Path to the CSV file.
    Returns:
        np.ndarray: Loaded data as a NumPy array.
    ...
    data = df.iloc[:30*100, 1:50].to_numpy()
    total_days = data.shape[0]
    num_steps = 30
    d = data.shape[1]
    data = data.reshape(int(total_days/num_steps), num_steps, d)
    print(data.shape)

    # Min-max normalization to [0, 100]
    arr_min = np.nanmin(data)  # Use nanmin if array may contain NaNs
    arr_max = np.nanmax(data)

    data = (data - arr_min) / (arr_max - arr_min) * 10

    return data


def load_weather_data(
        file_path: str = "real_data/all_six_datasets/weather/weather.csv"
    ) -> np.ndarray:
    ...
    data = df.iloc[:144*90, 1:11].to_numpy()
    num_steps = 72
    d = data.shape[1]
    total_samples = data.shape[0]
    num_trajectories = int(np.ceil(total_samples / num_steps))

    data = data.reshape(num_trajectories, num_steps, d)
    ...
    for i in range(data.shape[0]):
        if data[i, 0, 0] - data[i, -1, 0] > 0:
            retain.append(i)
    data = data[retain]
    print(data.shape, "after removing the increasing ones")

    return data


def load_real_data(data_name: str = "DGS10") -> np.ndarray:
    """Load real data from a file based on the data name.

    Args:
        data_name (str): Name of the data to load. Defaults to "DGS10".

    Returns:
        np.ndarray: Loaded data as a NumPy array.
    """
    if data_name == "DGS10":
        return load_fred_data("real_data/DGS10.csv")
    elif data_name == "molecule":
        data, skipped = load_molecule_data("real_data/Evolution_Trajectory/hanshi_data/feature1")
        return data
    elif data_name == "exchange_rate":
        return load_exchange_rate_data("real_data/all_six_datasets/exchange_rate/exchange_rate.csv")
    elif data_name == "electricity":
        return load_electricity_data("real_data/electricity/electricity.csv")
    elif data_name == "weather":
        return load_weather_data("real_data/all_six_datasets/weather/weather.csv")
    else:
        raise ValueError(f"Unknown data name: {data_name}")


if __name__ == "__main__":
    data_name = "weather"  # "molecule", "DGS10", "exchange_rate", "electricity", "weather"

    data = load_real_data(data_name=data_name)
    ...
    run_experiment(real_data=data, T=num_steps*0.05, d=d, dt=0.05, num_trajectories=num_trajectories,
                   version=5, max_iter=max_iter, known_initial_value=known_initial_value,
                   random_percent=random_percent)

    """
    Testing our method on real data:
    1) Load real data from .csv files, descriptions of those files will be in real_data/metadata.txt.
    2) Run the experiment with the loaded data.
    3) The results will be saved in the 'figures' folder.
    """